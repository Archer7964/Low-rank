...
f_eq_v = (
    (1.0 - 0.3) * np.exp(-0.5 * v**2)
    + 0.3 * np.exp(-0.5 * (v - 4.0) ** 2)
) / np.sqrt(2.0 * np.pi)
f_eq = np.ones((nx, 1)) * f_eq_v[None, :]
f_0 = (
    (1.0 - 0.3) * np.exp(-0.5 * v[None, :] ** 2)
    * (1.0 + alpha * np.cos(beta * x[:, None]))
    + 0.3 * np.exp(-0.5 * (v[None, :] - 4.0) ** 2)
    * (1.0 + alpha * np.sin(beta * x[:, None]))
) / np.sqrt(2.0 * np.pi)

K = np.arange(1, 6)
H_basis = np.vstack(
    (
        np.cos(beta * K[:, None] * x[None, :]),
        np.sin(beta * K[:, None] * x[None, :]),
    )
)
TOL = 1e-6
MAX_STEPS = 10
...
def optimize(a_k, h=1.0, tol=TOL, max_steps=MAX_STEPS):
...
def main(h=1.0, tol=TOL, max_steps=MAX_STEPS):
    a_k = np.zeros(len(H_basis))
